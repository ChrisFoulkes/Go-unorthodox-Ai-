...
def loadboardpositions():        
        ...
        for x in boards:
                ...
                if x == "}":
                        positions = False
                        smoves[move].loadmoves(loadmove)
                        move += 1
    ...
    def bestmove(self):
        ...
        if list1 != None: 
            for i in range(0, 10):  
                ...
                final_list.append(list1[maxpostion])                          
                list1.pop(maxpostion)
                
                              

            for listvalue in final_list:
                cord = str_coord(listvalue.pos.last)
                cordx = ord(cord[0])%32
                try :
                     cordy = cord[1] + cord[2]
                except IndexError:
                ...
                historyvalue = smoves[CURRENT_MOVE_COUNT].rows[(int(cordy)-1)][(int(cordx)-1)]
               
            
			for  listvalue in final_list:
					listvalue.combined_value = ((node.v/2) / minmaxadjust(listvalue.historyvalue))
					
            outputvalue = (max(final_list, key=lambda node: (node.combined_value)))

          
      
		
        return outputvalue if self.children is not None else None
...
def tree_descend(tree, amaf_map, disp=False):
    ...
    while nodes[-1].children is not None and passes < 2:
        ...
        node.v += 1
        #changes  mine to node values - 
        cord = str_coord(node.pos.last)
        cordx = ord(cord[0])%32
        try :
            cordy = cord[1] + cord[2]
        except IndexError:
        ...
        #print   (int(cordy)-1)
        #print   (int(cordx)-1)     
        historyvalue = smoves[CURRENT_MOVE_COUNT].rows[(int(cordx)-2)][(int(cordy)-1)]




        node.value = historyvalue
...
def tree_search(tree, n, owner_map, disp=False):
    ...
    incoming = []  # positioparse_coordns that finished evaluation
...
def dump_subtree(node, thres=N_SIMS/50, indent=0, f=sys.stderr, recurse=True):
    ...
    print("%s+- %s winrate %.3f (%d/%d, prior %d/%d, rave %d/%d=%.3f, urgency %.3f, history value %.1f)  " %
          (indent*' ', str_coord(node.pos.last), node.winrate(),
           ...
           node.rave_urgency(), node.value), file=f)
    if not recurse:
        return
...
#tree summary used for displaying tree information - need to add some form of output display
